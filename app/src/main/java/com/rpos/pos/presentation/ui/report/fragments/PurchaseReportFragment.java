...
public class PurchaseReportFragment extends SharedFragment {
    ...
    private void prepareExcel(){
        try {
            ...
            for (int i=0;i<invoiceEntityList.size();i++){
                ...
                cell.setCellValue(invoice.getDate().substring(0,10));
                cell.setCellStyle(cellbodyStyle);
            ...
            if(isSaved){
                ...
                String savedMessage = getString(R.string.report_saved) + " to SD Card Documents/Pos_Reports/Purchase";
    ...
    private boolean storeExcelInStorage(Context context, String fileName) {
        boolean isSuccess;
        //File file = new File(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), "Purchase/Report"); //new File(context.getExternalFilesDir(null), fileName);
        File file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS), "Pos_Reports/Purchase");
